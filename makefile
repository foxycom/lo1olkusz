...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.pjanczyk.lo1olkusz"
...
espresso-tests.log : app-original.apk app-androidTest.apk running-emulator
	...
	$(ADB) shell am instrument -w -r com.pjanczyk.lo1olkusz.test/android.support.test.runner.AndroidJUnitRunner | tee espresso-tests.log
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) stop-all-emulators
	$(RM) running-emulator
...
%.testlog: app-androidTest.apk app-instrumented.apk running-emulator
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c com.pjanczyk.lo1olkusz))
	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear com.pjanczyk.lo1olkusz; \
	else \
	...
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.pjanczyk.lo1olkusz.test/android.support.test.runner.AndroidJUnitRunner | tee $(@)
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces com.pjanczyk.lo1olkusz ./traces/$(TEST_NAME) force-clean
...
copy-shadows :
	cp -v ./shadows/*.java app/src/carvedTest/com/pjanczyk/lo1olkusz
